...
module.exports = {
    name: "/connect/account/view/command",
    /**
    * @param {request} req
    * @param {response} res
    */
    run: async (req, res) => {
        const paymentOfUser = await Promise.all((await Payment.getAll()).filter(x => x.account_id === req.user.account_id).map(async x => {
            ...
            return {
                ...
                statusInNumber: x.payment_status,
                create_time: formatDate(x.create_time),
                items: reformItems,
                payment_tracker: x.payment_tracker
            }
        }))
        res.status(200).render("connect/command", { data: { connect: req?.user, basket: req?.basket, paymentOfUser } })
    }
}