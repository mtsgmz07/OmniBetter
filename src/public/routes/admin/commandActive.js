...
module.exports = {
    name: "/admin/command/active",
    /**
    * @param {request} req
    * @param {response} res
    */
    run: async (req, res) => {
        const filter = req.query?.filter ? (!isNaN(Number(req.query?.filter)) && Number(req.query?.filter) >= 0 && Number(req.query?.filter) <= 3) ? Number(req.query?.filter) : 0 : 0
        let allPayment = await Promise.all((await Payment.getAll()).filter(x => x.payment_status === filter).map(async x => {
        ...
        res.status(200).render("admin/commandActive", { data: { connect: req.user, basket: req.basket, allPayment } })
    }
}